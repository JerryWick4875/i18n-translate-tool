...
export async function run() {
  const testName = 'Export Scattered: Basic Functionality';
  console.log(`\n🧪 Test: ${testName}`);
  ...
  try {
    ...
    if (result.totalCount !== 3) {
      console.log(`❌ ${testName} failed: expected totalCount to be 3, got ${result.totalCount}`);
      process.exit(1);
    }

    if (result.uniqueCount !== 3) {
      console.log(`❌ ${testName} failed: expected uniqueCount to be 3, got ${result.uniqueCount}`);
      process.exit(1);
    }
